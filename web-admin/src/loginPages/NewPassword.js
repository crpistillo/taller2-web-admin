...
import {
  SHOW_NEWPASS_ERROR_MESSAGE,
  SHOW_NEWPASS_SPINNER,
  SHOW_SUCCESSFUL_NEWPASS,
    ...
    this.formFields = [
      ...
      },
      {
        ...
        onChangeAction: (value) =>
          this.setState({ new_target_password: value }),
      },
    ];
...
const mapDispatchToProps = (dispatch) => {
  return {
    setErrorMessage: (value, message) => {
      dispatch({
        type: SHOW_NEWPASS_ERROR_MESSAGE,
        payload: { showErrorMessage: value, errorMessage: message },
      });
    },
    setSpinner: (value) =>
      dispatch({ type: SHOW_NEWPASS_SPINNER, payload: value }),

    setSuccessful: (value) =>
      dispatch({ type: SHOW_SUCCESSFUL_NEWPASS, payload: value }),
